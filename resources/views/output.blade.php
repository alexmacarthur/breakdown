        ...
        <table>
            <tbody>
                @foreach($timingLabels as $key => $label)
                    @if(isset($breakdown->timings->durations->$key))
                        <tr>
                            <td class="font-bold text-gray">{{ $label[1] }} {{ $label[0] }}</td>
                            <td class="font-bold {{ $label[2] }}">{{ number_format($breakdown->timings->breakdown->$key * 100, 1) }}%</td>
                            <td class="text-gray">{{ number_format($breakdown->timings->durations->$key, 2) }} {{ $breakdown->unit }}</td>
                        </tr>
                    @endif
                @endforeach
            </tbody>
    ...
    @if($breakdown->redirectCount > 0)
        <div>
            <h2 class="font-bold text-sky-500 uppercase mb-1">
                Redirects ({{ number_format($breakdown->redirectTime, 2) }}ms)
            </h2>

            <div class="ml-3 space-y-2">
                @foreach($breakdown->redirects as $index => $redirect)
                    <div>
                        <div class="mb-1">
                        ...
                        <div>
                            <details>
                                <summary class="text-gray">
                                    Response Headers ({{ is_array($redirect) ? count($redirect['headers']) : count($redirect->headers) }})
                                </summary>
                                <div class="mt-1">
                                    @foreach(is_array($redirect) ? $redirect['headers'] : $redirect->headers as $header)
                                        <div class="flex">
